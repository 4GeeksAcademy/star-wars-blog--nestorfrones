    ...
    return (
        <>
            {store.People.map((character, index) => {
                return (
                    <div key={character + index} className="card_people white card col-12 col-sm-6 col-md-3 mx-2 mt-5 shadow bg-body-tertiary rounded-3">
                        <Link to={"/details/" + (character.url.split("/")[4]) + "/" + (character.url.split("/")[5])} className="text-decoration-none">
                            <img onClick={() => setDetails(character)} src={"https://starwars-visualguide.com/assets/img/characters/" + (character.url.split("/")[5]) + ".jpg"} className="card-img-top" alt="..." />
                        </Link>
